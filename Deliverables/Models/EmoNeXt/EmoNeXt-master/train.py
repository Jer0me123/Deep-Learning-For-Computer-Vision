...
class Trainer:
    ...
    def val_epoch(self):
        ...
        print(
            "Eval loss: %.4f, Eval Accuracy: %.10f %%"
...
if __name__ == "__main__":
    ...
    parser.add_argument( # Additional Argumnet
        "--patience",
        type=int,
        default=15,
        help="The number of epochs without improvement before stopping the training",
    )
    parser.add_argument("--dataset-name", type=str, help="Name of the dataset used only for model naming convention")
    ...
    try:
        ...
        val_dataset = datasets.ImageFolder(opt.dataset_path + "/validation", val_transform)
    ...
    # Check class-to-index mapping in your dataset
    print("Class to index mapping:", train_dataset.class_to_idx)

    net = get_model(len(train_dataset.classes), opt.model_size, in_22k=opt.in_22k, class_weights=class_weights)
    ...
    Trainer(
        ...
        amp=opt.amp,
        early_stopping_patience=opt.patience, #Added patience as a modifiable parameter
    ).run()
