...
class EmoNeXt(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            loss = F.cross_entropy(logits, labels, weight=self.class_weights, label_smoothing=0.2) + attention_loss
...
def get_model(num_classes, model_size="tiny", in_22k=False, class_weights=None):
    if model_size == "tiny":
        depths = [3, 3, 9, 3]
        dims = [96, 192, 384, 768]
        url = (
    ...
    net = EmoNeXt(
        depths=depths, dims=dims, num_classes=default_num_classes, drop_path_rate=0.1, class_weights=class_weights
    )
